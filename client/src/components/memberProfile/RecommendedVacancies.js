...
import JobAppsModal from "../partnerProfile/JobAppsModal";
import { get, del } from "../../services/axios";
import VacanciesList from "../vacancies/List";
import { withRouter } from "react-router-dom";

class RecommendedVacancies extends Component {
  state = {
    loading: true,
    error: false,
    searchBar: "",
  ...
  componentDidMount() {
    const { id } = this.props;
    const url = "vacancies/getRecommendationsNormal/" + id;
    get(url)
      .then(vacancies => {
        this.setState({
    ...
    return (
      <Segment id="vacancy-segment" loading={loading} padded>
        ...
        <Header as="h1" textAlign="center">
          {myProfile ? "My Recommended Vacancies" : "Vacancies"}
...
export default withRouter(RecommendedVacancies);
